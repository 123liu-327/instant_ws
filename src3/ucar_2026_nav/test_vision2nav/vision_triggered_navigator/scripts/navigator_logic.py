...
def coverage_anchor_order(point_count, preferred_anchor=0, max_count=0):
    """Return a cyclic patrol order beginning at a one-based anchor number."""
    order = list(range(max(0, int(point_count))))
    preferred_idx = int(preferred_anchor) - 1
    if 0 <= preferred_idx < len(order):
        order = order[preferred_idx:] + order[:preferred_idx]
    if int(max_count) > 0:
        order = order[:int(max_count)]
    return order
...
def wall_frame_docking_command(normal_error, tangent_error, yaw_error,
                               ...
                               min_yaw=0.05,
                               translation_yaw_gate=0.22,
                               forward_yaw_gate=0.14,
                               forward_tangent_gate=0.12):
    """Blend the final wall-frame corrections for a mecanum base.

    A large heading error still gets rotation priority so the rectangular
    footprint does not sweep sideways.  Once roughly perpendicular, tangent,
    heading and normal errors converge together instead of forming three
    stop-start phases.  Forward motion uses tighter gates than lateral motion.
    """
    normal_error = float(normal_error)
    tangent_error = float(tangent_error)
    yaw_error = float(yaw_error)
    yaw_gate = max(abs(float(yaw_tolerance)),
                   abs(float(translation_yaw_gate)))
    forward_yaw_gate = max(abs(float(yaw_tolerance)),
                           abs(float(forward_yaw_gate)))
    forward_tangent_gate = max(abs(float(tangent_tolerance)),
                               abs(float(forward_tangent_gate)))

    command_yaw = bounded_axis_command(
        yaw_error, yaw_tolerance, 1.25, max_yaw, min_yaw)
    if abs(yaw_error) > yaw_gate:
        return 0.0, 0.0, command_yaw

    # Fade lateral motion in after the body is mostly perpendicular.  This
    # avoids a hard phase boundary while still protecting the long corners.
    yaw_blend = max(0.30, min(1.0, 1.0 - abs(yaw_error) / yaw_gate))
    command_y = bounded_axis_command(
        tangent_error, tangent_tolerance, 1.0, max_y, 0.025) * yaw_blend

    command_x = 0.0
    if (abs(yaw_error) <= forward_yaw_gate and
            abs(tangent_error) <= forward_tangent_gate):
        yaw_forward_blend = max(
            0.30, 1.0 - abs(yaw_error) / forward_yaw_gate)
        tangent_blend = max(
            0.35, 1.0 - abs(tangent_error) / forward_tangent_gate)
        command_x = bounded_axis_command(
            normal_error, normal_tolerance, 0.9, max_x, 0.03)
        command_x *= min(yaw_forward_blend, tangent_blend)
    return command_x, command_y, command_yaw
