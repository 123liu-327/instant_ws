...
def ocr_observation_quality(payload):
    ...
    points = []
    raw_bbox = (payload.get("merged_target_bbox") or
                payload.get("target_bbox") or payload.get("bbox") or ())
    ...
    width_ratio = min(1.0, bbox_width / image_width)
    height_ratio = min(1.0, bbox_height / image_height)
    center_x_ratio = min(1.0, max(0.0, center_x / image_width))
    center_y_ratio = min(1.0, max(0.0, center_y / image_height))
    edge_margin_ratio = min(
        min(xs) / image_width,
        (image_width - max(xs)) / image_width,
        min(ys) / image_height,
        (image_height - max(ys)) / image_height,
    )
    explicit_complete = payload.get("bbox_complete")
    bbox_complete = (
        bool(explicit_complete) if explicit_complete is not None
        else edge_margin_ratio >= 0.01)
    ...
    score = (
        ...
        + 1.50 * (1.0 - center_error_x)
        + 0.30 * (1.0 - center_error_y)
        + (0.50 if bbox_complete else 0.0)
        + 0.20 * confidence
    ...
    return {
        ...
        "bbox_edge_margin_ratio": edge_margin_ratio,
        "bbox_complete": bbox_complete,
        "observed_bbox": [list(point) for point in points],
...
class CompetitionFlow:
    def __init__(self):
        ...
        self.wait_for_start = bool_param("~wait_for_start", False)
        self.stage_exclusive_resources = bool_param(
            "~stage_exclusive_resources", True)
        self.task2_prewarm_enabled = bool_param(
            "~task2_prewarm_enabled", True)
        self.second_parking_prewarm_enabled = bool_param(
            "~second_parking_prewarm_enabled", True)
        ...
        if self.factory_sign_localization_mode != "off":
            from ucar_2026_competition import factory_sign_localization as localization_module
            globals().update({
                name: getattr(localization_module, name)
                for name in (
                    "bbox_metrics", "camera_bearing", "fit_wall_line",
                    "inverse_transform_point", "map_wall_fallback",
                    "observation_gate", "observation_quality",
                    "ray_wall_intersection", "scan_points_in_sector",
        ...
        if localization_module is None:
            self.factory_sign_room_walls = []
            self.factory_sign_fusion = None
        else:
            room_corners = rospy.get_param("~factory_sign_room_corners", [
                [-2.2311, -1.2505], [2.8000, -1.1940],
                [-2.2197, -3.2746], [2.7739, -3.2186],
            ])
            self.factory_sign_room_walls = localization_module.build_room_walls(
                room_corners)
            self.factory_sign_fusion = localization_module.FactorySignFusion(
                rospy.get_param("~factory_sign_localization_max_samples", 24))
        ...
        self._ensure_handoff_scan_subscription()
        rospy.Subscriber(
            rospy.get_param(
        ...
        if self.wait_for_start:
            rospy.Service("/competition/start", Trigger, self._start_cb)
        rospy.on_shutdown(self.shutdown)

        self.move_base = actionlib.SimpleActionClient("/move_base", MoveBaseAction)
        rospy.logwarn(
            "CONTROLLER_RUNTIME python=%s localization=%s",
            sys.executable, self.factory_sign_localization_mode)
    ...
    def _publish_workflow_manifest(self):
        ...
        stages = {
            ...
            "task2": {
                "flow": [
                    "stage-exclusive factory OCR", "room anchor navigation",
        ...
        payload = {
            ...
            "stages": stages,
            "camera_policy": {
                "front_stages": "/usb_cam/image_raw on demand",
                "ucar_camera_start_stage": "task4 after stop line",
                "ucar_camera_front_stage_subscription": False,
            },
            "resource_policy": {
                "stage_exclusive": self.stage_exclusive_resources,
                "task2_prewarm": self.task2_prewarm_enabled,
                "second_parking_prewarm": self.second_parking_prewarm_enabled,
                "voice_input": "task1 command only",
                "spark_llm": "task1 reasoning only",
                "factory_ocr_navigator": "one parking operation at a time",
            },
        }
        ...
        rospy.logwarn(
            "CAMERA_POLICY /usb_cam/image_raw starts at first visual stage; "
            "/ucar_camera/image_raw remains unopened until task4 reaches "
            "the stop line")

    def _ensure_handoff_scan_subscription(self):
        if self.handoff_scan_sub is not None:
            return
        self.handoff_scan_sub = rospy.Subscriber(
            "/scan", LaserScan, self._handoff_scan_cb,
            queue_size=1, buff_size=1024 * 1024, tcp_nodelay=True)

    def _release_handoff_scan_subscription(self, reason):
        if not self.stage_exclusive_resources:
            return
        subscriber = self.handoff_scan_sub
        self.handoff_scan_sub = None
        if subscriber is not None:
            try:
                subscriber.unregister()
            except Exception as exc:
                rospy.logwarn("STAGE_RESOURCE scan unsubscribe failed: %s", exc)
        with self.lock:
            self.qr_scan = None
            self.qr_scan_received_at = 0.0
            self.localization_scan_samples.clear()
        rospy.loginfo("STAGE_RESOURCE_STOP resource=controller_scan reason=%s", reason)
    ...
    def _remember_simulation_observation(self, ocr_payload, confirmed):
        ...
        with self.lock:
            ...
            previous_complete = (
                bool(previous.get("bbox_complete", False))
            ...
            current_complete = bool(observation.get("bbox_complete", False))
            should_replace = (
                previous is None or
                (current_complete and not previous_complete) or
                (current_complete == previous_complete and
        ...
        self.publish_status(
            ...
            "[CACHE] best simulation={} anchor={} area={:.3f} size=({:.2f},{:.2f}) "
            "center=({:.2f},{:.2f}) complete={} score={:.2f}".format(
                self.sim_category,
                current_anchor,
                float(observation["area_ratio"]),
                float(observation["bbox_width_ratio"]),
                float(observation["bbox_height_ratio"]),
                float(observation["bbox_center_x_ratio"]),
                float(observation["bbox_center_y_ratio"]),
    ...
    def _start_cb(self, _req):
        ...
        if self.start_event.is_set():
            return TriggerResponse(False, "competition already started")
        self.start_event.set()
        return TriggerResponse(True, "front-stage flow start requested")
    ...
    def task1_task2_handoff(self):
        ...
        self._release_handoff_scan_subscription("task1_task2_handoff_complete")
    ...
    def production_task4_handoff(self, source_stage):
        ...
        self._release_handoff_scan_subscription("task4_handoff_complete")
    ...
    def _ensure_front_camera(self):
        ...
        if proc is not None and proc.poll() is None:
            return
        self.start_child(
            "front_camera", "ucar_2026_competition",
            "front_usb_cam_stage_v1.launch", {"debug": self.debug})
        try:
            rospy.wait_for_message("/usb_cam/image_raw", Image, timeout=6.0)
        except rospy.ROSException as exc:
            self.stop_child("front_camera")
            raise StageError("front USB camera did not become ready: {}".format(exc))
        rospy.loginfo("STAGE_RESOURCE_START resource=front_camera")

    def _ensure_smart_llm(self):
        proc = self.children.get("smart_llm")
        if proc is not None and proc.poll() is None:
            return
        self.start_child(
            "smart_llm", "ucar_2026_competition",
            "smart_factory_llm_stage_v1.launch")
        rospy.loginfo("STAGE_RESOURCE_START resource=smart_factory_llm")

    def _retire_ros_nodes(self, names, reason):
        ...
        try:
            ...
            rospy.loginfo(
                "STAGE_RESOURCE_STOP resources=%s reason=%s", targets, reason)
        except Exception as exc:
            rospy.logwarn(
                "STAGE_RESOURCE stop failed resources=%s reason=%s error=%s",
                targets, reason, exc)

    @staticmethod
    def _trim_process_memory(reason):
        gc.collect()
        try:
        ...
        try:
            ...
            pass
        rospy.loginfo(
            "STAGE_MEMORY reason=%s controller_rss_mb=%.1f",
            reason, rss_kb / 1024.0 if rss_kb >= 0 else -1.0)
    ...
    def stop_child(self, key):
        ...
        try:
            ...
            if log_handle is not None:
                log_handle.close()
            if self.stage_exclusive_resources and key in (
                    "qr_decoder", "smart_llm", "factory_ocr",
                    "factory_navigator"):
                self._trim_process_memory("child_stopped:{}".format(key))
    ...
    def _run_qr_keyframe_round(
        ...
        """Rotate, confirm odometry stopped, then sample one stationary keyframe."""
        self.publish_status(
            "task1", status_state,
            "[QR] keyframe round {} started: {:.1f}-degree steps; count={}/{}"
            .format(round_index, math.degrees(step_angle), self._qr_count(),
                    expected_count))
        for round_step in range(1, step_count + 1):
            ...
            self._rotate_qr_step(
                step_angle, speed, direction, stale_sec, scan_deadline,
                step_margin)
            self._settle_for_qr(
    ...
    def scan_qr_at_current_pose(self, status_state):
        ...
        # observations there. Moving camera frames are discarded by the QR node.
        ...
        try:
            ...
            self._run_qr_keyframe_round(
                1, first_round_steps, step_angle, speed, direction, settle_sec,
                expected_count, stale_sec, scan_deadline, step_margin,
                status_state)
            if self._qr_count() < expected_count:
            ...
            if self._qr_count() < expected_count:
                self.publish_status(
                    "task1", status_state,
                    "[QR] starting {:.1f}-degree-offset, {:.1f}-degree stationary "
                    "round 2".format(
                        second_round_offset_deg, second_round_step_deg))
                self._rotate_qr_step(
                    second_round_offset, speed, direction, stale_sec,
                    scan_deadline, step_margin)
                self._settle_for_qr(
                    settle_sec, expected_count, scan_deadline, stale_sec,
                    "QR round 2 offset")
                self._run_qr_keyframe_round(
                    2, second_round_steps, second_round_step_angle, speed, direction,
                    settle_sec, expected_count, stale_sec, scan_deadline,
                    step_margin, status_state)
                if self._qr_count() < expected_count:
                    self._wait_for_qr_network_idle(
    ...
    def _factory_ocr_args(self):
        return {
            ...
            # Commit 39ff room OCR reports every workshop.  Mission-level
            # voting in this controller decides which category may park.
            "target_category": "",
            "enable_speech": False,
            "required": True,
        }
    ...
    def _prewarm_task2_stack(self):
        if "task2" not in stage_sequence(self.mode, self.enable_simulation):
            return False
        if not self.task2_prewarm_enabled:
            self.publish_status(
                "task1", "task2_prewarm_skipped",
                "task2 prewarm disabled; OCR/navigation start after task1")
            return False
        if not self._factory_ocr_is_running():
            ...
            self.start_child(
                ...
                "factory_sign_ppocr_rknn_39ff_room_v1.launch",
        ...
        if navigator is None or navigator.poll() is not None:
            self.start_child(
                ...
                "vision_triggered_navigator_39ff_room_v1.launch",
    ...
    def _prewarm_second_parking_stack(self):
        ...
        if not self.second_parking_prewarm_enabled:
            return False
        simulation_category = normalize_category(self.sim_category)
    ...
    def _second_parking_anchor_choice(self):
        ...
        if self.cached_sim_observation:
            ...
            if anchor > 0:
                return anchor, "best_visual_simulation"
    ...
    def task1(self):
        ...
        if self.stage_exclusive_resources:
            self._retire_ros_nodes(
                ("/src3_legacy_voice_bridge",),
        ...
        if self.stage_exclusive_resources:
            self.stop_child("smart_llm")
            rospy.loginfo(
                "STAGE_RESOURCE_STOP resource=smart_factory_llm reason=reasoning_complete")
        ...
        if not wait_for_announcement:
            ...
            while not rospy.is_shutdown() and time.monotonic() < deadline:
                ...
                rospy.sleep(0.05)
        if ("task2" in stage_sequence(self.mode, self.enable_simulation)
                and self.task2_prewarm_enabled):
    ...
    def _run_factory_parking(self, item, workshop, announcement_text,
        ...
        parking_fallback_used = False
        parking_phase_entered_at = None
        parking_progress_at = None
        parking_last_status = ""
        ...
        parking_failure_states = frozenset((
            ...
            "parking_obstacle_blocked", "memory_parking_failed",
        ))

        def parking_state_has_started(status):
            return (
                status in (
                    "triggered", "target_locked", "target_centering",
                    "target_geometry_locking", "target_geometry_locked",
                    "centered",
                )
                or status.startswith("memory_parking_")
                or status.startswith("parking_")
            )

        def accept_parking_fallback(reason):
            nonlocal parking_succeeded, parking_fallback_used
            parking_succeeded = True
            parking_fallback_used = True
            self.safe_stop(cancel_navigation=True)
            self.stop_child("factory_navigator")
            self.navigator_status = "arrived"
            self.publish_status(
                "task2", "parking_fail_open_completed",
                "parking {} accepted as completed after parking-stage "
                "failure: {}".format(parking_index, reason))
            rospy.logwarn(
                "PARKING_FAIL_OPEN_COMPLETED index=%d item=%s workshop=%s "
                "reason=%s",
                parking_index, item, workshop, reason)
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                now_monotonic = time.monotonic()
                current_status = self.navigator_status
                if current_status != parking_last_status:
                    parking_last_status = current_status
                ...
                if center_only and current_status == "centered":
                    break
                parking_started = (
                    parking_phase_entered_at is not None
                    or self.trigger_acknowledged)
                if current_status in parking_failure_states and parking_started:
                    accept_parking_fallback(
                        "navigator_status={}".format(current_status))
                    break
                if current_status == "failed" and parking_started:
                    accept_parking_fallback(
                        "navigator_status=failed after target lock")
                    break
                ...
                if parking_started:
                    if parking_phase_entered_at is None:
                        parking_phase_entered_at = now_monotonic
                    if parking_progress_at is None:
                        parking_progress_at = now_monotonic
                    ...
                    if (stalled_for >= parking_stall_timeout or
                        ...
                        accept_parking_fallback(
                            "parking stalled {:.1f}s in state={} "
                            "(total {:.1f}s)".format(
                                stalled_for, current_status or "unknown",
                                parking_elapsed))
                        break
                for key in ("factory_navigator", "factory_ocr"):
                    ...
                    if proc and proc.poll() is not None:
                        if parking_started:
                            accept_parking_fallback(
                                "{} exited with code {} in state={}".format(
                                    key, proc.returncode,
                                    current_status or "unknown"))
                            break
                        raise StageError("{} exited unexpectedly with code {}".format(key, proc.returncode))
                if parking_fallback_used:
                    break
                rospy.sleep(0.1)
            else:
                if parking_phase_entered_at is not None or self.trigger_acknowledged:
                    accept_parking_fallback(
                        "factory navigation timeout after {:.1f}s".format(timeout))
                else:
                    raise StageError(
                        "factory navigation timed out after {:.1f}s".format(timeout))
            parking_succeeded = (
        ...
        if parking_index == 1 and not center_only:
            if not self.second_parking_prewarm_enabled:
                ...
                self.stop_child("factory_navigator")
                self.publish_status(
                    "task2", "first_parking_resources_released",
                    "OCR/navigation stopped before parking 1 announcement")
            else:
                try:
                    self.stop_child("factory_navigator")
    ...
    def task3(self):
        if self.stage_exclusive_resources:
            self.stop_child("front_camera")
            rospy.loginfo(
                "STAGE_RESOURCE_STOP resource=front_camera reason=simulation_stage")
        if not self.sim_category:
    ...
    def run(self):
        try:
            if self.wait_for_start:
                self.publish_status(
                    "competition", "armed",
                    "vehicle held stopped; call /competition/start to begin")
                last_stop_at = 0.0
                while not self.start_event.wait(0.05):
                    self.check_abort()
                    now = time.monotonic()
                    if now - last_stop_at >= 0.25:
                        self.safe_stop(cancel_navigation=True)
                        last_stop_at = now
                self.check_abort()
                self.publish_status(
                    "competition", "starting",
                    "front-stage start gate released")
            handlers = {
                "task1": self.task1,
                "task2": self.task2,
                "task3": self.task3,
