...
class FactorySignPPOCRRknnNode:
    def __init__(self) -> None:
        ...
        self.inference_rate = float(rospy.get_param("~inference_rate", 7.0))
        ...
        self.view_index = 0
        self.tracking_scale_enabled = ros_bool(
            rospy.get_param("~tracking_scale_enabled", True), True)
        self.tracking_scale_hold_sec = max(
            0.5, float(rospy.get_param("~tracking_scale_hold_sec", 3.0)))
        self.tracking_view_scale = None
        self.tracking_scale_until = 0.0
    ...
    def _process_once(self, frame, image_stamp=None) -> None:
        now = time.time()
        if (self.tracking_scale_enabled and
                self.tracking_view_scale is not None and
                now < self.tracking_scale_until):
            view_scale = self.tracking_view_scale
        else:
        ...
        if confirmed and self.tracking_scale_enabled:
            scale_changed = self.tracking_view_scale != view_scale
            self.tracking_view_scale = view_scale
            self.tracking_scale_until = now + self.tracking_scale_hold_sec
            if scale_changed:
                self.rospy.logwarn(
                    "FACTORY_OCR_TRACK_SCALE category=%s scale=%.2f hold=%.1fs",
                    confirmed, view_scale, self.tracking_scale_hold_sec)
