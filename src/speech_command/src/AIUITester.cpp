...
void TestListener::onEvent(const IAIUIEvent &event) const
{
	...
	case AIUIConstant::EVENT_WAKEUP:
	{
    	...
    	int ret = system("/home/ucar/ucar_ws/src/speech_command/start.sh");  // & 表示后台运行

   
    	if (ret == 0) {
        	cout << ">>>>> 脚本执行成功！" << endl;
    	} else {
        	cerr << ">>>>> 脚本执行失败，错误码: " << ret << endl;
    	}
	}
	break;
...
void AIUITester::createAgent()
{
	...
	if (reader.parse(fileParam, paramJson, false))
	{
		...
		if (wakeup_mode == "ivw")
		{
			...
			//std::string lgiparams = "appid=e9f954db,engine_start=ivw,ivw_res_path=fo|/home/ucar/ucar_ws/src/speech_command/config/AIUI/ivw/wakeupresource.jet";
			...
			string ivw_lib_path = "/home/ucar/ucar_ws/src/speech_command/lib/arm64/libmsc.so";

			paramJson["ivw"]["msc_lib_path"] = ivw_lib_path;
			cout<<paramJson["ivw"]["res_path"]<<endl;
			cout<<paramJson["ivw"]["msc_lib_path"]<<endl;
		}
		//end
