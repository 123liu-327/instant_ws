...
required_files=(
  "$SOURCE_ROOT/ucar_2026_competition/launch/full_competition_legacy_flowend_v1.launch"
  "$SOURCE_ROOT/ucar_2026_competition/launch/traffic_flowend_handoff_v1.launch"
  "$SOURCE_ROOT/ucar_2026_competition/scripts/competition_flow_flowend_v1.py"
...
echo "[INFO] New flow: legacy voice + Spark + src6 navigation/OCR/stop-line + ucar camera + traffic_light_vision + flow_end."
...
if [[ "$DRY_RUN_VALUE" == "1" ]]; then
  ...
  roslaunch --nodes ucar_2026_competition \
    full_competition_legacy_flowend_v1.launch \
...
cleanup() {
  ...
  local stale_patterns=(
    ...
    "roslaunch factory_sign_ppocr_rknn_test factory_sign_ppocr_rknn_src6_hybrid_v1.launch"
    "/factory_sign_ppocr_rknn_test/scripts/factory_sign_ppocr_rknn_src6_hybrid_v1.py"
    "roslaunch vision_triggered_navigator vision_triggered_navigator_src6_points_v1.launch"
    "/vision_triggered_navigator/scripts/vision_triggered_navigator_src6_points_v1.py"
...
set +e
roslaunch ucar_2026_competition full_competition_legacy_flowend_v1.launch \
  enable_simulation:="$ENABLE_SIMULATION_VALUE" \
  sim_bridge_host:="$SIM_BRIDGE_HOST_VALUE" \
