...
class ContinuousQRHybrid(object):
    def __init__(self):
        ...
        self.complete_hold_s = max(
            0.0, float(rospy.get_param("~complete_hold_s", 0.10)))
    ...
    def run(self):
        ...
        self.publish_summary()
        # Summary/status are latched and the persistent wrapper keeps the
        # camera alive, so a two-second post-result sleep only delays handoff.
        rospy.sleep(self.complete_hold_s)
        return 0
