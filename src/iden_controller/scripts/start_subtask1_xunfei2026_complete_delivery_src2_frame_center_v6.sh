...
CONFLICT_PATTERN='^/(amcl|map_server|move_base|base_driver|my_base_driver|ydlidar_node|ucar_camera|cloud_asr_test2|follow_test|reference_line_follow_|xunfei2026_|subtask1_|factory_room_ocr|factory_sign_ppocr_rknn_node)'
...
echo "[INFO] Current dual-stage flow: src2 d-point navigation + exclusive locked legacy v1 parking"
cleanup_before_start
FILTER_PATTERN='check crc16 faild|header_crc8 error|check frame end|head_len error|ANCHOR_CONTINUOUS_SWEEP anchor=|FACTORY_OCR_SCANNING|OCR_TARGET_VOTE_SUSPECT_HOLD|OCR_PHYSICAL_SIGN_SUSPECT_HOLD|SUSPECT_TARGET_HOLD|OCR_TARGET_FRAMING_WAIT|OCR_TARGET_VERTICAL_CLIP_ACCEPTED|OCR_NON_TARGET_VIEW_IGNORED|PARKING_OCR_CENTER_JUMP_FILTERED|IN_PROCESS_PARKING_ALIGN wall=|IN_PROCESS_PARKING_CONE_BLOCK_FORWARD_ONLY|IN_PROCESS_PARKING_FRONT_CORNER_LATERAL_SLOW|PREPARK_ALIGN center=|WALL_CORNER_CMD'
set +e
FLOW_STARTED=1
rosrun iden_controller xunfei2026_launch_src2_frame_center_v6.py "$@" 2>&1 \
