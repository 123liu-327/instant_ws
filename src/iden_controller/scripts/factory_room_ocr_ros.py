...
class FactoryRoomOCRNode(object):
    ...
    @staticmethod
    def suspect_bbox(crops):
        """Return the largest paper/text crop even before OCR classifies it."""
        candidates = []
    ...
    def process_once(self):
        ...
        try:
            ...
            suspect_bbox = self.suspect_bbox(crops)
            payload = {
                ...
                "bbox": bbox,
                # A crop has already passed the detector's strict white-paper,
                # dark-text and geometry filters.  Publish it separately so
                # the motion manager can pause and gather more OCR frames even
                # when the recognizer has not selected a workshop yet.
                "factory_sign_suspect": bool(suspect_bbox is not None),
                "suspect_bbox": suspect_bbox,
