...
ENTRY="$SCRIPT_DIR/start_subtask1_xunfei2026_complete_delivery_src2_dual_stage_v5.sh"
FLOW_PID=""
OVERLAY_PID=""
cleanup() {
  set +e
...
(
  ...
  while (( SECONDS < DEADLINE )); do
    ...
    if timeout 1.20 rosnode list 2>/dev/null \
      ...
      exec roslaunch iden_controller xunfei2026_src2_line_follow_takeover_v3.launch \
        mode:="$MODE"
    fi
    sleep 0.25
  done
