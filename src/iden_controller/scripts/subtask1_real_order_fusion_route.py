...
class Subtask1RealOrderFusion(object):
    def __init__(self):
        ...
        self.qr_scan_status_topic = rospy.get_param(
            "~qr_scan_status_topic", "/xunfei2026_continuous_qr/status")
        ...
        self.spark_warmup_enabled = as_bool(rospy.get_param(
            "~spark_warmup_enabled", True))

        self.lock = threading.Lock()
        self.spark_http_lock = threading.Lock()
        self.spark_session = None
        self.spark_warmup_started = False
        self.spark_warmup_done = False
        ...
        rospy.Subscriber(
            self.qr_scan_status_topic, String,
            self.qr_scan_status_callback, queue_size=10)
        rospy.Subscriber(
    ...
    def qr_scan_status_callback(self, msg):
        status = normalize_text(msg.data).upper()
        if status != "CONTINUOUS_SCAN_STARTED":
            return
        with self.lock:
            if (not self.spark_warmup_enabled or self.spark_warmup_started or
                    self.decision_done):
                return
            self.spark_warmup_started = True
        threading.Thread(target=self.spark_warmup_thread, daemon=True).start()

    def spark_warmup_thread(self):
        """Warm the authenticated endpoint while the chassis scans QR codes."""
        if not self.spark_ready():
            rospy.logwarn("SPARK_WARMUP_SKIPPED reason=missing_key_or_disabled")
            return
        rospy.logwarn("SPARK_WARMUP_START trigger=qr_rotation")
        started = time.time()
        data = self.call_spark(
            "这是连接预热请求。只返回严格JSON：{\"ready\":true}。",
            max_tokens=12)
        ok = bool(self.extract_spark_content(data))
        with self.lock:
            self.spark_warmup_done = ok
        rospy.logwarn(
            "SPARK_WARMUP_%s elapsed=%.2fs session_reused=true",
    ...
    def call_spark(self, prompt, max_tokens=120):
        ...
        try:
            ...
            started = time.time()
            # A shared Session preserves the authenticated TCP/TLS connection
            # established by the QR-rotation warmup.  Calls are serialized;
            # the real/simulation decisions are sequential in the active flow.
            with self.spark_http_lock:
                if self.spark_session is None:
                    self.spark_session = requests.Session()
                resp = self.spark_session.post(
