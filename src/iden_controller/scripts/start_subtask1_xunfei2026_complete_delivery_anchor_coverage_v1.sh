...
CONFLICT_PATTERN='^/(amcl|map_server|move_base|base_driver|my_base_driver|ydlidar_node|ucar_camera|cloud_asr_test2|follow_test|xunfei2026_|subtask1_|factory_room_ocr)'
...
echo "[INFO] Complete flow: voice -> first nav -> 3 unique QR -> Spark/TTS -> ordered anchor coverage -> unchanged OCR -> improved parking -> simulation handoff"

cleanup_before_start

set +e
FLOW_STARTED=1
roslaunch iden_controller subtask1_xunfei2026_complete_delivery_anchor_coverage_v1.launch "$@" 2>&1 \
