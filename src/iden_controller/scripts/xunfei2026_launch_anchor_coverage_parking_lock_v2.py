#!/usr/bin/python3
"""Launch the existing anchor flow with v3 manager and handoff substituted.

The original launch file is read-only.  ROS launch XML is loaded from memory,
so this creates neither a modified legacy launch file nor a temporary copy.
"""
...
NEW_MANAGER = "xunfei2026_room_delivery_dual_stage_v3.py"
...
def set_node_param(node, name, value):
    matches = [
        ...
        if (child.nodeType == child.ELEMENT_NODE and
            child.tagName == "param" and
            child.getAttribute("name") == name)
    ]
    if len(matches) != 1:
        raise RuntimeError(
            "expected one private param {} in node {}, found {}".format(
...
def load_launch_xml(overrides):
    ...
    if len(manager_nodes) != 1:
        raise RuntimeError(
            "expected exactly one {} node, found {}".format(
                OLD_MANAGER, len(manager_nodes)))
    manager_nodes[0].setAttribute("type", NEW_MANAGER)

    # move_base only needs to reach a safe camera observation envelope.  The
    # visual stop-line controller owns the final lateral and heading accuracy.
    # The old 20-degree gate rejected the reported 21.7-degree endpoint before
    # that controller was even started.
    handoff_nodes = [
        node for node in document.getElementsByTagName("node")
        if node.getAttribute("name") == "xunfei2026_simulation_handoff"
    ]
    if len(handoff_nodes) != 1:
        raise RuntimeError(
            "expected one simulation handoff node, found {}".format(
                len(handoff_nodes)))
    handoff = handoff_nodes[0]
    if handoff.getAttribute("type") != OLD_HANDOFF:
        raise RuntimeError(
            "unexpected simulation handoff type: {}".format(
...
def parse_overrides(arguments):
    ...
    for argument in arguments:
        if ":=" not in argument:
            raise RuntimeError(
                "launch arguments must use name:=value: {}".format(argument))
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write("[ERROR] parking-lock launch failed: {}\n".format(exc))
        sys.exit(1)
