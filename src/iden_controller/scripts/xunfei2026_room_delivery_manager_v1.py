...
class Xunfei2026RoomDeliveryManager(object):
    def __init__(self):
        ...
        self.candidate_blank_grace_s = float(rospy.get_param(
            "~candidate_blank_grace_s", 1.2))
        self.sign_suspect_confirm_frames = max(2, int(rospy.get_param(
            "~sign_suspect_confirm_frames", 2)))
        ...
        self.non_target_view_cooldown_s = max(0.0, float(rospy.get_param(
            "~non_target_view_cooldown_s", 3.0)))
        ...
        self.parking_planner_recovery_enabled = bool(rospy.get_param(
            "~parking_planner_recovery_enabled", False))
        self.parking_planner_recovery_attempts = max(1, int(
            rospy.get_param("~parking_planner_recovery_attempts", 2)))
        self.parking_planner_stall_command = max(0.04, float(
            rospy.get_param("~parking_planner_stall_command_m", 0.12)))
        self.parking_planner_stall_motion = max(0.008, float(
            rospy.get_param("~parking_planner_stall_motion_m", 0.025)))
        self.parking_planner_recovery_shift = max(0.10, float(
            rospy.get_param("~parking_planner_recovery_shift_m", 0.22)))
        self.parking_planner_recovery_backoff = max(0.05, float(
            rospy.get_param("~parking_planner_recovery_backoff_m", 0.10)))
        self.parking_planner_recovery_timeout = max(3.0, float(
            rospy.get_param("~parking_planner_recovery_timeout_s", 8.0)))
        ...
        self.parking_stall_requested_vy = 0.0
        self.parking_stall_pixel_error = 0.0
    ...
    def publish_state(self, state, **values):
        ...
        # Keep the ROS topic machine-readable and complete, but make the
        # operator terminal concise.  High-detail consumers should subscribe
        # to status_topic instead of parsing console output.
        details = []
        for key in ("anchor", "goal", "target", "delivery_stage", "frames",
                    "result", "reason", "attempt", "wall_distance"):
            if key in values and values[key] not in (None, ""):
                details.append("{}={}".format(key, values[key]))
        if all(key in values for key in (
                "nominal_x", "nominal_y", "x", "y")):
            nominal_x = float(values["nominal_x"])
            nominal_y = float(values["nominal_y"])
            actual_x = float(values["x"])
            actual_y = float(values["y"])
            details.append(
                "nominal=({:.2f},{:.2f}) actual=({:.2f},{:.2f}) "
                "delta=({:+.2f},{:+.2f})".format(
                    nominal_x, nominal_y, actual_x, actual_y,
                    actual_x - nominal_x, actual_y - nominal_y))
        elif all(key in values for key in (
                "requested_x", "requested_y", "selected_x", "selected_y")):
            requested_x = float(values["requested_x"])
            requested_y = float(values["requested_y"])
            selected_x = float(values["selected_x"])
            selected_y = float(values["selected_y"])
            details.append(
                "nominal=({:.2f},{:.2f}) actual=({:.2f},{:.2f}) "
                "delta=({:+.2f},{:+.2f})".format(
                    requested_x, requested_y, selected_x, selected_y,
                    selected_x - requested_x, selected_y - requested_y))
        suffix = " | " + " ".join(details) if details else ""
        rospy.logwarn("ROOM %s | %s -> %s%s", state,
                      self.selected_item or "-",
                      self.target_warehouse or "-", suffix)
    ...
    def ocr_callback(self, msg):
        ...
        if confirmed_non_target:
            with self.lock:
                ...
                if self.non_target_view_label != stable_label:
                    ...
                    self.non_target_view_locked_at = now
                    self.non_target_target_frames = 0
                    rospy.logwarn(
        ...
        with self.lock:
            ...
            locked_at = self.non_target_view_locked_at
        ...
        if ignored_label:
            ...
            cooldown_elapsed = (
                locked_at <= 0.0 or
                now - locked_at >= self.non_target_view_cooldown_s)
            hard_released = (
                ...
                odom_yaw_delta >= self.non_target_hard_release_yaw)
            soft_released = (
                cooldown_elapsed and blank_long_enough and
            ...
            released = hard_released or soft_released
            with self.lock:
            ...
            if not released:
                rospy.logwarn_throttle(
                    ...
                    "cooldown_remaining=%.1fs scan_continues=true",
                    ...
                    max(0.0, self.non_target_view_cooldown_s -
                        max(0.0, now - locked_at)))
                return
            with self.lock:
                self.non_target_view_label = ""
                self.non_target_view_anchor = None
        ...
        # Candidate handling is intentionally more sensitive than final
        # parking.  There are two ways to pause:
        #   1) the requested workshop is leading the vote window but has not
        #      yet reached the final vote threshold;
        #   2) two consecutive frames contain a detector-qualified physical
        #      paper/text sign, but OCR has not classified it yet.
        # The robot remains stopped while OCR keeps running.  Only
        # ``current_target`` can increment the final confirmation counter and
        # set target_event, so an uncertain/non-target sign cannot start
        # parking by itself.
        suspect_bbox = payload.get("suspect_bbox")
        suspect_usable = False
        if (bool(payload.get("factory_sign_suspect", False)) and
                isinstance(suspect_bbox, (list, tuple)) and
                len(suspect_bbox) >= 4 and width > 1.0 and height > 1.0):
            sx0, sy0, sx1, sy1 = [float(value) for value in suspect_bbox[:4]]
            suspect_visible_height = max(
                0.0, min(sy1, height) - max(sy0, 0.0))
            suspect_usable = (
                sx1 > sx0 and sy1 > sy0 and
                sx0 >= 0.01 * width and sx1 <= 0.99 * width and
                suspect_visible_height >= max(
                    18.0, self.target_min_visible_height_ratio * height))
        with self.lock:
            self.sign_suspect_count = (
                self.sign_suspect_count + 1 if suspect_usable else 0)
            suspect_count = self.sign_suspect_count

        target_vote_candidate = (
            frame_label == target and votes >= self.candidate_ocr_votes and
            bbox_usable)
        physical_sign_candidate = (
            suspect_usable and
            suspect_count >= self.sign_suspect_confirm_frames)
        candidate_confirm_frames = max(2, self.target_confirm_frames - 1)
        confirmed_candidate = (
            current_target and confirm_count >= candidate_confirm_frames)
        candidate_now = (
            confirmed_candidate or target_vote_candidate or
            physical_sign_candidate)
        if (candidate_now and now >= self.candidate_cooldown_until):
            with self.lock:
                self.candidate_last_seen = now
                # Only target-labelled evidence may be used later for visual
                # alignment.  A generic physical suspect merely stops motion.
                if frame_label == target and bbox_usable:
                    existing = self.parking_target_ocr
                    existing_verified = (
                        isinstance(existing, dict) and
                        bool(existing.get("stable", False)) and
                        existing.get("label") == target and
                        existing.get("frame_label") == target and
                        int(existing.get("votes", 0) or 0) >=
                        self.required_ocr_votes)
                    incoming_verified = (
                        stable and stable_label == target and
                        frame_label == target and
                        votes >= self.required_ocr_votes)
                    # Once stationary OCR has raised target_event, retain the
                    # last fully verified target.  The OCR node still emits
                    # interleaved candidate frames while the robot settles;
                    # those stable=False frames must not erase the snapshot
                    # used by the lidar wall-lock and parking handoff.
                    if (incoming_verified or not existing_verified or
                            not self.target_event.is_set()):
                        self.parking_target_ocr = dict(payload)
                        self.parking_target_stamp = now
            self.candidate_event.set()
            if physical_sign_candidate and not target_vote_candidate:
                rospy.logwarn_throttle(
                    0.5, "OCR_PHYSICAL_SIGN_SUSPECT_HOLD count=%d/%d "
                    "frame=%s votes=%d bbox=%s continue_ocr=true",
                    suspect_count, self.sign_suspect_confirm_frames,
                    frame_label or "unknown", votes, str(suspect_bbox))
            elif target_vote_candidate and not current_target:
                rospy.logwarn_throttle(
                    0.5, "OCR_TARGET_VOTE_SUSPECT_HOLD target=%s votes=%d/%d "
                    "final=%d stable=%s bbox=%s continue_ocr=true",
                    target, votes, self.candidate_ocr_votes,
                    self.required_ocr_votes, str(stable), str(bbox))
        elif (not current_target and
              now - self.candidate_last_seen > self.candidate_blank_grace_s):
            self.candidate_event.clear()
    ...
    def planner_reposition_after_parking_stall(self, attempt):
        """Use move_base to escape a blocked direct lateral alignment.

        The candidate goals remain on the target-wall side of the room, but
        include a small backoff so TEB can curve around a cone instead of
        repeating an in-place body-y command against it.
        """
        self.smooth_stop_robot()
        pose = self.current_pose()
        if pose is None:
            self.publish_state(
                "PARKING_PLANNER_RECOVERY_NO_POSE", attempt=attempt)
            return False
        target_yaw = (pose[2] if self.parking_target_wall_yaw is None else
                      self.parking_target_wall_yaw)
        requested_vy = self.parking_stall_requested_vy
        if abs(requested_vy) < 1.0e-4:
            requested_vy = -self.parking_stall_pixel_error
        lateral_sign = 1.0 if requested_vy >= 0.0 else -1.0
        tangent_x = -math.sin(target_yaw)
        tangent_y = math.cos(target_yaw)
        normal_x = math.cos(target_yaw)
        normal_y = math.sin(target_yaw)
        base_shift = self.parking_planner_recovery_shift
        base_backoff = self.parking_planner_recovery_backoff
        # Prefer the requested centring side.  Smaller/larger alternatives let
        # the global planner select a path around a randomly placed cone.
        offsets = (
            (base_shift, base_backoff),
            (0.70 * base_shift, 1.35 * base_backoff),
            (1.35 * base_shift, 1.60 * base_backoff),
            (0.55 * base_shift, 2.00 * base_backoff),
        )
        candidates = []
        for shift, backoff in offsets:
            x = (pose[0] + lateral_sign * shift * tangent_x -
                 backoff * normal_x)
            y = (pose[1] + lateral_sign * shift * tangent_y -
                 backoff * normal_y)
            if (self.room_min_x + 0.12 <= x <= self.room_max_x - 0.12 and
                    self.room_min_y + 0.12 <= y <= self.room_max_y - 0.12):
                candidates.append((x, y, target_yaw, shift, backoff))

        self.cone_control_pub.publish(Bool(data=True))
        self.ocr_control_pub.publish(String(data="enable"))
        self.clear_and_wait("parking lateral stall planner recovery")
        for index, candidate in enumerate(candidates, 1):
            x, y, yaw, shift, backoff = candidate
            if not self.make_plan_exists(x, y, yaw):
                continue
            self.publish_state(
                "PARKING_PLANNER_RECOVERY_NAVIGATION",
                attempt=attempt, candidate=index, x=x, y=y, yaw=yaw,
                lateral_shift=shift, backoff=backoff,
                requested_vy=self.parking_stall_requested_vy,
                pixel_error=self.parking_stall_pixel_error)
            result = self.send_goal(
                x, y, yaw, "parking_stall_recovery_{}_{}".format(
                    attempt, index),
                min(self.goal_timeout, self.parking_planner_recovery_timeout),
                watch_target=False)
            self.smooth_stop_robot()
            if result != "SUCCEEDED":
                self.publish_state(
                    "PARKING_PLANNER_RECOVERY_CANDIDATE_FAILED",
                    attempt=attempt, candidate=index, result=result)
                self.clear_and_wait(
                    "parking planner candidate {} {}".format(index, result))
                continue
            self.publish_state(
                "PARKING_PLANNER_RECOVERY_POSE_REACHED",
                attempt=attempt, candidate=index)
            # Rotation-only visual alignment is safe here.  The following
            # pre-parking translation is again executed by move_base.
            if not self.align_target_for_parking():
                self.publish_state(
                    "PARKING_PLANNER_RECOVERY_ALIGNMENT_FAILED",
                    attempt=attempt)
                return False
            if not self.refresh_centered_target_wall():
                self.publish_state(
                    "PARKING_PLANNER_RECOVERY_WALL_LOCK_FAILED",
                    attempt=attempt)
                return False
            if not self.approach_target():
                self.publish_state(
                    "PARKING_PLANNER_RECOVERY_PREPARK_FAILED",
                    attempt=attempt)
                return False
            return True
        self.publish_state(
            "PARKING_PLANNER_RECOVERY_NO_REACHABLE_GOAL", attempt=attempt)
        return False
    ...
    def park(self):
        ...
        no_progress_count = 0
        lateral_request_distance = 0.0
        lateral_request_signed_distance = 0.0
        lateral_request_pose = self.odom_pose
        lateral_request_stamp = time.monotonic()
        ...
        while not rospy.is_shutdown() and time.monotonic() < align_deadline:
            ...
            command_dt = max(0.0, min(0.20, now - lateral_request_stamp))
            lateral_request_stamp = now
            if abs(self.parking_lateral_requested) >= self.parking_min_vy:
                lateral_request_distance += (
                    abs(self.parking_lateral_requested) * command_dt)
                lateral_request_signed_distance += (
                    self.parking_lateral_requested * command_dt)
            lateral_motion = 0.0
            if lateral_request_pose is not None and odom_pose is not None:
                lateral_motion = math.hypot(
                    odom_pose[0] - lateral_request_pose[0],
                    odom_pose[1] - lateral_request_pose[1])
            if lateral_motion >= self.parking_planner_stall_motion:
                lateral_request_distance = 0.0
                lateral_request_signed_distance = 0.0
                lateral_request_pose = odom_pose
            elif (self.parking_planner_recovery_enabled and
                  lateral_request_distance >=
                  self.parking_planner_stall_command):
                self.smooth_stop_robot()
                with self.lock:
                    self.parking_active = False
                self.parking_stall_requested_vy = (
                    lateral_request_signed_distance
                    if abs(lateral_request_signed_distance) >= 0.01 else
                    self.parking_lateral_requested)
                self.parking_stall_pixel_error = pixel_error
                self.publish_state(
                    "IN_PROCESS_PARKING_LATERAL_STALL_REPLAN",
                    commanded_distance=lateral_request_distance,
                    actual_motion=lateral_motion,
        ...
        try:
            ...
            while not rospy.is_shutdown() and time.monotonic() < deadline:
                ...
                while (parking_result == "REPLAN" and
                       planner_attempt <
                       self.parking_planner_recovery_attempts and
                ...
                if parking_result == "REPLAN":
                    self.publish_state(
                        "PARKING_PLANNER_RECOVERY_EXHAUSTED",
