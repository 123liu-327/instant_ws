...
def test_preferred_anchor_continues_in_cyclic_scan_order():
    assert coverage_anchor_order(9, preferred_anchor=6) == [5, 6, 7, 8, 0, 1, 2, 3, 4]
...
def test_reordered_rounded_nine_anchor_order_is_preserved_without_offsets():
    calibrated = [
        (-0.70, -2.82, -1.56),
        (-1.68, -2.79, -3.12),
        (-1.66, -2.28, -3.14),
...
def test_logged_real_car_docking_pose_is_safe_to_accept_and_announce():
    errors = (0.032, 0.003, 0.044)
    assert docking_within_tolerance(errors, 0.035, 0.015, 0.05)

    diagnostics = parking_footprint_margins(
        (0.292, -0.003, math.pi - 0.044),
        (0.0, 0.0),
        (1.0, 0.0),
        0.50,
        0.50,
    ...
    assert diagnostics["inside"]
    assert min(
        diagnostics["near_margin"],
        diagnostics["far_margin"],
        diagnostics["side_margin"],
    ) >= 0.02
...
def test_wall_frame_controller_blends_small_errors_but_guards_large_yaw():
    guarded = wall_frame_docking_command(
        0.20, 0.10, 0.30, 0.02, 0.03, 0.05,
        0.12, 0.08, 0.22, 0.05)
    assert guarded == (0.0, 0.0, 0.22)

    blended = wall_frame_docking_command(
        0.20, -0.10, 0.10, 0.02, 0.03, 0.05,
        0.12, 0.08, 0.22, 0.05)
    assert 0.0 < blended[0] <= 0.12
    assert -0.08 <= blended[1] < 0.0
    assert 0.0 < blended[2] <= 0.22

    forward = wall_frame_docking_command(
        0.20, 0.01, 0.02, 0.02, 0.03, 0.05,
        0.12, 0.08, 0.22, 0.05)
    assert 0.0 < forward[0] <= 0.12
    assert forward[1:] == (0.0, 0.0)
