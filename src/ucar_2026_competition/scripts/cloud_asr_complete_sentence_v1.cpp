...
// The complete-sentence variant keeps AIUI alive across endpointed fragments.
// Shutdown is requested only after the ROS bridge accepts both categories.
static bool g_close_requested = false;

// AIUI may end one utterance before the complete competition command has
// reached ROS. Re-arm only after AIUI is idle, giving the bridge time to
// accept a complete inline command first.
static int g_aiui_state = -1;
static bool g_rearm_pending = false;
static ros::WallTime g_rearm_requested_at;
...
void commandAcceptedCallback(const std_msgs::Bool::ConstPtr& msg)
{
    if (msg->data)
    {
        ...
        ROS_WARN("ASR_COMPLETE_SENTENCE_ACCEPTED; microphone shutdown requested");
    }
}
...
void ttsCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    const bool expects_followup =
        msg->data.find("请说明需要领取的货品大类") != std::string::npos;
    ...
    if (expects_followup && g_agent)
    {
        // TTS runs synchronously in the ROS callback, so no microphone frames
        // are fed to AIUI while the prompt is playing.  Resume the existing
        // continuous session immediately instead of destroying the agent and
        // paying the two-second process startup cost for every dialogue turn.
        g_final_text.clear();
        g_task_sent = false;
        ...
        IAIUIMessage* wakeupMsg = IAIUIMessage::create(
            AIUIConstant::CMD_WAKEUP, 0, 0, "", nullptr);
        g_agent->sendMessage(wakeupMsg);
        wakeupMsg->destroy();
        cout << "🎙️ 提示播报完成，AIUI原进程立即继续收音" << endl;
    }
}
...
int main(int argc, char** argv) {
    ...
    sub_command_accepted = nh.subscribe(
        command_accepted_topic,
        2,
        commandAcceptedCallback);
    ROS_INFO_STREAM("TTS subscriber listening on " << tts_topic);
    ROS_WARN_STREAM(
        "ASR_COMPLETE_SENTENCE_READY fragments=/factory/voice_raw_text accepted="
        << command_accepted_topic);
...
cout<<"🚀 已发送CMD_WAKEUP"<<endl;
    ...
    if(g_need_destroy && g_agent)
    {
        if(!g_final_text.empty())
        {
            ...
            g_rearm_requested_at = ros::WallTime::now();
        }
        g_need_destroy = false;
    }

    if(g_close_requested && g_agent)
    {
        g_rearm_pending = false;
        g_asr_enable = false;
        ...
        cout << "✅ 完整双类别指令已接收，AIUI关闭" << endl;
    }

    if(g_rearm_pending && !g_close_requested && g_agent)
    {
        const double elapsed =
            (ros::WallTime::now() - g_rearm_requested_at).toSec();
        if(elapsed >= 0.20 && (g_aiui_state == 3 || elapsed >= 0.80))
        {
            IAIUIMessage* wakeupMsg = IAIUIMessage::create(
                AIUIConstant::CMD_WAKEUP, 0, 0, "", nullptr);
            g_agent->sendMessage(wakeupMsg);
            wakeupMsg->destroy();
            g_rearm_pending = false;
            ROS_WARN("ASR_IDLE_REARMED; waiting for remaining command");
        }
    }
